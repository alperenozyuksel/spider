...
class p_gym:
    ...
    def return_obv_whole(self):
        ...
        # PyBullet'ten dönen link verilerini alın
        link_data = p.getLinkStates(self.spiderbot_id, list(range(self.num_of_joints)), computeLinkVelocity=1)

        # Verileri işleyerek numpy dizisine dönüştürün
        link_positions = [state[0] for state in link_data]  # Pozisyon bilgileri
        link_velocities = [state[6] for state in link_data]  # Hız bilgileri

        # Pozisyon ve hızları numpy dizilerine dönüştürün
        link_positions_array = np.array(link_positions)
        link_velocities_array = np.array(link_velocities)

        # Daha sonra bu dizileri istediğiniz şekilde kullanabilirsiniz

        # obtain observations for each joint of spiderbot
        joint_data = p.getJointStates(self.spiderbot_id, list(range(self.num_of_joints)))

        # Pozisyon ve hız verilerini ayrı olarak işlememiz gerekiyor
        joint_positions = [state[0] for state in joint_data]  # Pozisyon verileri
        joint_velocities = [state[1] for state in joint_data]  # Hız verileri

        # Pozisyon ve hızları numpy dizilerine dönüştürün
        joint_positions_array = np.array(joint_positions)
        joint_velocities_array = np.array(joint_velocities)

        # Eğer diğer verilere de ihtiyacınız varsa onları da aynı şekilde işleyebilirsiniz
